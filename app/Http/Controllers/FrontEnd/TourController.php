...
class TourController extends Controller
{
    ...
    {
        $tourCategories = BlogTypes::take(5)->get();
    ...
    public function search(Request $request)
    {
        if ($request->has('search_query')) {
            $tours = Tours::with('chitiettour', 'loaitour', 'chuongtrinhtour')
            ...
            $category = BlogTypes::where('tenloai', $request->category)->firstOrFail();

            $tours = Tours::with('loaitour')->where('maloaitour', $category->maloaitour)
